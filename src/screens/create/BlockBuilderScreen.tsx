...
export function BlockBuilderScreen({
  templateId,
  reviewMode = false,
  onBrandPress,
  onBack,
  ...
  const { user } = useAuth();
  const [draft, setDraft] = useState<BlockTemplateInput>(defaultBlockDraft);
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: loadError } = await getBlockTemplate(templateId);
    setLoading(false);
    if (loadError || !data) {
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    setError(null);
    const { id, error: saveError } = await saveBlockTemplate({
      userId: user.id,
      templateId: savedId,
      draft,
    });
  ...
  const onRemoveConfirm = async () => {
    ...
    const result =
      removeMode === 'hard'
        ? await deleteBlockTemplate(savedId, user.id)
        : await archiveBlockTemplate(savedId, user.id);
    setRemoving(false);
    if (result.error) {
      setRemoveMode(null);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <EditorChrome reviewLockId={reviewMode ? LOCK_ROOT.block : undefined}>
