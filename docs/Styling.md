...
`InsightsQueryBuilderScreen` — Query builder. **Read (shipped):** `Qb*` nest chrome (warm `qbTokens` by depth), LockedOutline + LockedPreviewModal. **Author (next):** madlib / clause compose over the same nest AST — not five Session edit shells. Contract: [`Insights_Query_Builder.md`](./Insights_Query_Builder.md). Gold: [`references/workout-builder/`](./references/workout-builder/) + [`references/query-builder/`](./references/query-builder/).
